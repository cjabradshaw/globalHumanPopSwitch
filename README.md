...
## Inequality analysis
The main analysis script includes global and selected regional World Inequality Database (WID) Gini analyses against global temperature anomaly, ecological footprint, and CO2-e emissions. It also fits resampled boosted regression trees for each global change variable using population size, per-capita consumption, and each global WID Gini definition as predictors. For Sub-Saharan Africa, WID Latin America, and WID MENA, it sources annual country primary-energy consumption from Our World in Data and ecological footprint and population from the Global Footprint Network, then writes population-weighted regional energy use and total/per-capita footprint to `data/WID_regional_energy_footprint.csv`. It also sums Climate Watch territorial greenhouse-gas sectors, supplied through Our World in Data, to create regional CO2-e totals and per-capita values in `data/WID_regional_emissions.csv`; these data cover 1990--2023. The pipeline fits regional total-footprint and CO2-e BRTs with regional population, energy use, and each regional Gini definition as predictors. Regional temperature-anomaly models await an aligned response series. Download the WID bulk files separately and set `WID_DATA_DIR` to the directory containing `WID_data_XX.csv` before running the script. The pipeline uses published WID series for the World, Sub-Saharan Africa, Latin America, and MENA; it does not average national Gini coefficients to construct regions that lack an aligned WID aggregate.
